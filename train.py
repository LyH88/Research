...
def train(args, model, device, train_loader, optimizer, epoch, logger):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = F.binary_cross_entropy(output[:,0], target.float())
...
def test(args, model, device, test_loader, logger):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            #pred = 1- output.max(1, keepdim=True)[1]
            ...
            print(output)
            correct += pred.eq(target.view_as(pred)).sum().item()
            test_loss += F.binary_cross_entropy(output[:, 0], target.float()).item()  # sum up batch loss
...
def validation(args, model, device, valid_loader, logger):
    ...
    with torch.no_grad():
        for data, target in valid_loader:
            ...
            pred = 1 - output.max(1, keepdim=True)[1]  # get the index of the max log-probability
            correct += pred.eq(target.view_as(pred)).sum().item()
            valid_loss += F.binary_cross_entropy(output[:, 0], target.float()).item()  # sum up batch loss
